#!/usr/bin/env python3
"""
Wait-count baseline: logistic regression on density of "wait" tokens in reasoning.

Produces two OOF probability columns using problem-level 5-fold CV:
  wait_within  — CV run separately inside each dataset
  wait_cross   — CV run across the full corpus

Usage:
    python -m reasonops.prediction.baseline_wait_count \
...
def main():
    ...
    with opener_r(args.corpus, "rt") as f:
        for line in f:
            ...
            if correct is None:
                continue
            text = r.get("reasoning") or r.get("model_response") or ""
            words = text.split()
            n_words = max(len(words), 1)
            feat = len(WAIT_PATTERN.findall(text)) / n_words
            records.append({
                "trace_id":   r["trace_id"],
    ...
    with opener_w(args.output, "wt") as out:
        for i, r in enumerate(records):
            out.write(json.dumps({
                ...
                "correct":      r["correct"],
                "wait_within":  None if np.isnan(within[i]) else float(within[i]),
                "wait_cross":   None if np.isnan(cross[i])  else float(cross[i]),
            }) + "\n")
