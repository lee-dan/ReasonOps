#!/usr/bin/env python3
"""
Length baseline: logistic regression on log(1 + len(reasoning)).

Produces two OOF probability columns using problem-level 5-fold CV:
  length_within  — CV run separately inside each dataset
  length_cross   — CV run across the full corpus

Usage:
    python -m reasonops.prediction.baseline_length \
    ...
    with opener_w(args.output, "wt") as out:
        for i, r in enumerate(records):
            out.write(json.dumps({
                ...
                "correct":       r["correct"],
                "length_within": None if np.isnan(within[i]) else float(within[i]),
                "length_cross":  None if np.isnan(cross[i])  else float(cross[i]),
            }) + "\n")
